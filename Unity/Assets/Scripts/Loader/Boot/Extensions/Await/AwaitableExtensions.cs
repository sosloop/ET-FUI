...
namespace UGFExtensions.Await
{
    public static partial class AwaitableExtensions
    {
        ...
        /// 加载资源（可等待）
        /// </summary>
        public static Task<object> LoadAssetAsync(this ResourceComponent resourceComponent, Type type,string assetName)
        {
#if UNITY_EDITOR
            TipsSubscribeEvent();
#endif
            TaskCompletionSource<object> loadAssetTcs = new TaskCompletionSource<object>();
            resourceComponent.LoadAsset(assetName, type, new LoadAssetCallbacks(
                (tempAssetName, asset, duration, userdata) =>
                {
                    var source = loadAssetTcs;
                    loadAssetTcs = null;
                    ...
                    {
                        Debug.LogError($"Load asset failure load type is {asset.GetType()} but asset type is {type}.");
                        source.SetException(new GameFrameworkException(
                            $"Load asset failure load type is {asset.GetType()} but asset type is {type}."));
                    }
                },
                (tempAssetName, status, errorMessage, userdata) =>
                {
